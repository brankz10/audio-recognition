...
from src.parts.features import FeatureFactory
import random
...
class JasperEncoderDecoder(nn.Module):
    ...
    def forward(self, x):
        if self.jasper_encoder.use_conv_mask:
            ...
            return self.jasper_decoder(t_encoded_t), t_encoded_len_t
        else:
            if self.transpose_in:
                x = x.transpose(1, 2)
            t_encoded_t = self.jasper_encoder(x)
            return self.jasper_decoder(t_encoded_t)
...
class MaskedConv1d(nn.Conv1d):
    ...
    def get_seq_len(self, lens):
        return (
            lens + 2 * self.padding[0] - self.dilation[0] * (self.kernel_size[0] - 1) - 1
        ) / self.stride[0] + 1
